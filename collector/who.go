...
func fetchSessionInfo(session, sessionId string) *WhoLogged {
	...
	out, err := exec.CommandContext(ctx, "loginctl", "show-session", sessionId,
		"-p", "Name", "-p", "Class", "-p", "TTY", "-p", "RemoteHost", "-p", "RemoteAddress").CombinedOutput()
	if err != nil {
		if session == "new" {
	...
	if !parseLoginCtlOutput(bytes.NewReader(out), w) {
		return nil
	}
	return w
}

func parseLoginCtlOutput(b *bytes.Reader, w *WhoLogged) bool {
