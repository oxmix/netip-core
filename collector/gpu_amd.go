...
func (c *Collector) collectGpuAmd() {
	...
	for i := 0; i < 10; i++ {
		...
		if len(data) < 128 {
			log.Printf("[collector] warn: gpu_metrics too small: %d bytes", len(data))
			continue
		}
...
func (c *Collector) parseGpuAmd(card uint8, data []byte) (GpuAmd, error) {
	...
	memUsed, memUsedErr := c.readUint64FromFile(fmt.Sprintf("/sys/class/drm/card%d/device/mem_info_vram_used", card))
	memTotal, memTotalErr := c.readUint64FromFile(fmt.Sprintf("/sys/class/drm/card%d/device/mem_info_vram_total", card))
	var memFree uint64
	if memUsedErr == nil && memTotalErr == nil && memTotal >= memUsed {
