...
var gpuNvidiaParserOnce sync.Once

func (c *Collector) startGpuNvidiaParser() {
	gpuNvidiaParserOnce.Do(func() {
		go func() {
			for data := range chanGpuNvidia {
				c.parseGpuNvidia(data)
			}
		}()
	})
}
...
func (c *Collector) collectGpuNvidia() {
	c.startGpuNvidiaParser()

	backoff := time.Second
	const maxBackoff = time.Minute

	for {
		ok, fatal := c.runGpuNvidiaOnce()
		if fatal {
			// nvidia-smi unavailable, stop
			return
		}
		...
		log.Printf("[collector] nvidia-smi exited, restarting in %s", backoff)
		time.Sleep(backoff)
...
func (c *Collector) runGpuNvidiaOnce() (ok bool, fatal bool) {
	start := time.Now()
	...
	if err != nil {
		log.Println("[collector] nvidia-smi stdout pipe err:", err)
		return false, false
	}

	if err = cmd.Start(); err != nil {
		if execErr, ok2 := errors.AsType[*exec.Error](err); ok2 && errors.Is(execErr.Err, exec.ErrNotFound) {
			return false, true
		}

		log.Println("[collector] nvidia-smi start err:", err)
		return false, false
	}
	...
	for {
		...
		if buff.Len() > 3*1e6 {
			log.Println("[collector] warn: over buffer gpu stats, forced flush")
			buff.Reset()
		}
	}
	...
	return time.Since(start) > 30*time.Second, false
}
...
type NvidiaSmiLog struct {
	...
	Gpu           []struct {
		...
		GpuPowerReadings struct {
			PowerState          string `xml:"power_state"`
			PowerDraw           string `xml:"power_draw"`
			InstantPowerDraw    string `xml:"instant_power_draw"` // since NVIDIA-SMI 580.126.09 / nvsmi_device_v13.dtd
...
func (c *Collector) parseGpuNvidia(data string) []SmiNvidia {
	...
	if err != nil {
		log.Println("[collector] smi unmarshal err:", err)
		return nil
	}
	...
	for _, g := range smi.Gpu {
		...
		if power == "" && powerLimit == "" {
			// struct since NVIDIA-SMI 550.54.14
			power = g.GpuPowerReadings.PowerDraw
			powerLimit = g.GpuPowerReadings.CurrentPowerLimit
		}
		if power == "" {
			// struct since NVIDIA-SMI 580.126.09 / xml nvsmi_device_v13.dtd
			power = g.GpuPowerReadings.InstantPowerDraw
		}
